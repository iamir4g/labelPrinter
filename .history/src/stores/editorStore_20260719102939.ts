...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  past: [],
  future: [],
  setTemplate: (template) => set({ template, selectedElementId: null, past: [], future: [] }),
  ...
  addText: () => {
    ...
    const next: Template = { ...template, elements: [...template.elements, el] }
    get().commit(next)
  ...
  deleteSelected: () => {
    ...
    const next: Template = { ...template, elements: template.elements.filter((el) => el.id !== id) }
    get().commit(next)
