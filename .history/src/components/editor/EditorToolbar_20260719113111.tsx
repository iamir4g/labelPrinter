    ...
    <div className="border-b border-zinc-900 bg-zinc-950/80 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-6 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className={cn(buttonClass(), !canUndo && "opacity-50")}
            disabled={!canUndo}
            onClick={() => undo()}
          >
          ...
          <button
            className={cn(buttonClass(), !canRedo && "opacity-50")}
            disabled={!canRedo}
            onClick={() => redo()}
          >
            ...
            onClick={async () => {
              ...
              if (w)
                writePrintDocument(
                  w,
                  dataUrl,
                  template.label.widthMm,
                  template.label.heightMm,
                );
            }}
          >
          ...
          <div className="hidden items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900/30 px-3 py-2 md:flex">
            ...
            <input
              ...
              value={zoom}
              onChange={(e) =>
                setZoom(clampNumber(Number(e.target.value), 0.25, 2))
              }
            />
            <span className="w-12 text-right text-xs text-zinc-300">
              {roundTo(zoom * 100, 0)}%
            </span>
          </div>
        </div>
