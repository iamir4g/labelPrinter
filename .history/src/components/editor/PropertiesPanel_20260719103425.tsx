...
import { AlignCenter, AlignLeft, AlignRight, Lock, Trash2, Unlock } from "lucide-react"
...
export default function PropertiesPanel() {
  const { template, selectedElementId, commit, updateElement, updateTextHtml, updateTextStyle, deleteSelected } = useEditorStore()
      ...
      <div className="flex h-full flex-col">
        <div className="border-b border-zinc-900 px-5 py-4">
          ...
          <div className="mt-2 space-y-1">
            {labelLine("سایز", `${template.label.widthMm}×${template.label.heightMm} mm`)}
        ...
        <div className="min-h-0 flex-1 overflow-auto px-5 py-4">
          <div className="space-y-6">
            <section className="space-y-3">
              <div className="text-xs font-medium text-zinc-300">تنظیمات لیبل</div>
              <div className="grid grid-cols-2 gap-3">
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onBlur={(e) => {
                      const widthMm = roundTo(clampNumber(parseNum(e.target.value, template.label.widthMm), 5, 200), 2)
                      commit({ ...template, label: { ...template.label, widthMm } })
                    }}
                  />
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-zinc-800 bg-zinc-950/30 px-3 py-2 text-sm text-zinc-100 focus:border-zinc-600 focus:outline-none"
                    onBlur={(e) => {
                      const heightMm = roundTo(clampNumber(parseNum(e.target.value, template.label.heightMm), 5, 200), 2)
                      commit({ ...template, label: { ...template.label, heightMm } })
                    }}
                  />
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-zinc-800 bg-zinc-950/30 px-3 py-2 text-sm text-zinc-100 focus:border-zinc-600 focus:outline-none"
                    onBlur={(e) => {
                      const dpi = clampNumber(parseNum(e.target.value, template.label.dpi), 72, 600)
                      commit({ ...template, label: { ...template.label, dpi } })
                    }}
                  />
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-zinc-800 bg-zinc-950/30 px-3 py-2 text-sm text-zinc-100 focus:border-zinc-600 focus:outline-none"
                    onBlur={(e) => {
                      const safeMarginMm = roundTo(clampNumber(parseNum(e.target.value, template.label.safeMarginMm), 0, 20), 2)
                      commit({ ...template, label: { ...template.label, safeMarginMm } })
                    }}
                  />
                </label>
              </div>
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-xs font-medium text-zinc-300">عنصر انتخاب‌شده</div>
                {selected ? (
                  <button
        ...
        <div className="border-t border-zinc-900 px-5 py-4">
          <div className="text-xs text-zinc-500">برای چاپ دقیق: در Print سیستم Scale را روی 100% بگذار و Fit-to-page را خاموش کن.</div>
        </div>
      </div>
    </div>
  )
...
function SelectedElementPanel({ selected }: { selected: TemplateElement }) {
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <label className="space-y-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-zinc-800 bg-zinc-950/30 px-3 py-2 text-sm text-zinc-100 focus:border-zinc-600 focus:outline-none"
            onBlur={(e) => updateElement(selected.id, { wMm: roundTo(clampNumber(parseNum(e.target.value, selected.wMm), 1, template.label.widthMm), 2) })}
          />
        </label>
        <label className="space-y-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-zinc-800 bg-zinc-950/30 px-3 py-2 text-sm text-zinc-100 focus:border-zinc-600 focus:outline-none"
            onBlur={(e) => updateElement(selected.id, { hMm: roundTo(clampNumber(parseNum(e.target.value, selected.hMm), 1, template.label.heightMm), 2) })}
          />
        </label>
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="space-y-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-zinc-800 bg-zinc-950/30 px-3 py-2 text-sm text-zinc-100 focus:border-zinc-600 focus:outline-none"
                onBlur={(e) => updateTextStyle(selected.id, { fontSizePx: clampNumber(parseNum(e.target.value, selected.data.fontSizePx ?? 14), 6, 200) })}
              />
            </label>
            <label className="space-y-1">
